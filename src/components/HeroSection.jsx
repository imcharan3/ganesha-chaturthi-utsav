...
import { 
  ...
  Instagram, QrCode, X, ExternalLink, Wallet, Camera
} from 'lucide-react';
...
export const HeroSection = ({ stats, settings, auction, onOpenDonation, setActiveTab }) => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: settings?.utsavName || 'Vijaya Colony Ganesha Diaries 2026',
        text: 'Join us in celebrating Ganesh Chaturthi 2026 with Vijaya Colony Ganesha Diaries! Follow @vijayacolony_ganesha_diaries, check events, donate, and participate in live laddu auction:',
  ...
  return (
    <div className="relative overflow-hidden pt-4 pb-12 sm:pt-8 sm:pb-16 space-y-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 text-center lg:text-left space-y-5">
            ...
            <div className="flex items-center justify-center lg:justify-start gap-2 text-xs sm:text-sm text-amber-300/90 bg-black/30 p-2.5 rounded-xl border border-amber-500/20">
              ...
              <span>{settings?.location || 'Vijaya Colony Mandapam, Main Road'}</span>
            </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3">
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-3.5 rounded-2xl bg-gradient-to-r from-amber-700/60 to-orange-800/60 hover:from-amber-600/70 hover:to-orange-700/70 border border-amber-400/50 text-amber-200 font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-gold"
              >
                <Camera className="w-4 h-4 text-yellow-300" />
                <span>Memories (జ్ఞాపకాలు 📸)</span>
              </button>
          ...
          <div className="lg:col-span-4 flex flex-col items-center justify-center">
            ...
            <div className="mt-3 sm:mt-4 inline-flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold text-amber-300/90 bg-amber-950/50 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border border-amber-500/30">
              <span>🌺 విజయ కాలనీ గణేష్ డైరీస్ 🌺</span>
            </div>
          </div>
        ...
        {/* OFFICIAL INSTAGRAM PAGE & QR HUB CARD */}
        <div className="temple-card p-5 sm:p-7 rounded-3xl border-2 border-pink-500/40 bg-gradient-to-br from-[#2a091d] via-[#1c0803] to-[#200615] relative overflow-hidden shadow-2xl space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            
            {/* Left Info: Instagram Brand & Announcement */}
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
              ...
              <div className="space-y-1.5">
                <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-pink-500/20 text-pink-300 border border-pink-500/30 text-xs font-bold">
                  <Instagram className="w-3.5 h-3.5 text-pink-400" />
                  <span>OFFICIAL INSTAGRAM COMMUNITY PAGE</span>
                </div>
                
                <h3 className="text-xl sm:text-3xl font-extrabold text-white tracking-tight flex items-center justify-center sm:justify-start gap-2">
                  <span>@vijayacolony_ganesha_diaries</span>
                </h3>
                
                <p className="text-xs sm:text-sm text-pink-200/90 max-w-xl leading-relaxed">
                  📢 <strong>ప్రతి తాజా అప్‌డేట్ మన ఇన్‌స్టాగ్రామ్ పేజీలో పోస్ట్ చేయబడుతుంది!</strong> ఉత్సవాల ప్రత్యక్ష పూజలు, దర్శనం ఫోటోలు, సాంస్కృతిక కార్యక్రమాలు & రీల్స్ కోసం ఇప్పుడే ఫాలో అవ్వండి.
                </p>
              </div>
            </div>

            {/* Right: Instagram QR Scanner & Follow CTA */}
            <div className="flex flex-wrap items-center justify-center gap-3 shrink-0">
              
              {/* Tap-to-Zoom QR Thumbnail */}
              <div 
                onClick={() => setShowInstaQrModal(true)}
                className="cursor-pointer bg-white p-1.5 rounded-2xl shadow-xl border-2 border-pink-400/70 hover:scale-105 transition-transform flex flex-col items-center gap-1 group"
                title="Click to zoom Instagram QR Code"
              >
                <img 
                  src="/instagram_qr.jpg" 
                  alt="Instagram Handle QR Code" 
                  className="w-20 h-20 sm:w-24 sm:h-24 object-contain rounded-xl" 
                />
                <span className="text-[10px] font-extrabold text-purple-900 group-hover:underline flex items-center gap-0.5">
              ...
              <div className="flex flex-col gap-2">
                <a
                  ...
                  className="px-5 py-3 rounded-2xl bg-gradient-to-r from-pink-600 via-rose-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white font-extrabold text-xs sm:text-sm shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all"
                >
                ...
                <button
                  onClick={() => setShowInstaQrModal(true)}
                  className="px-4 py-2 rounded-xl bg-pink-950/60 hover:bg-pink-900/60 border border-pink-500/40 text-pink-200 text-xs font-semibold flex items-center justify-center gap-1.5"
                >
      ...
      {/* FULLSCREEN INSTAGRAM QR MODAL */}
      {showInstaQrModal && (
        <div 
          className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200"
          onClick={() => setShowInstaQrModal(false)}
        >
          <div 
            className="relative w-full max-w-sm bg-gradient-to-b from-[#240e1c] via-[#1c0803] to-[#120502] border-2 border-pink-500/60 rounded-3xl p-6 text-center shadow-2xl space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setShowInstaQrModal(false)}
              className="absolute top-4 right-4 p-1.5 rounded-full bg-black/50 text-pink-200 hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <h3 className="font-devotional text-lg font-bold text-white">
              Scan Instagram QR Code
            </h3>
            
            <p className="text-xs text-pink-200/80">
              Open your Phone Camera or Instagram App to scan & follow our official page:
            </p>

            {/* QR Code Container */}
            <div className="bg-white p-3 rounded-2xl shadow-2xl mx-auto inline-block border-4 border-pink-500/50">
              <img 
                src="/instagram_qr.jpg" 
                alt="Instagram Handle QR Code" 
            ...
            <div className="pt-2 space-y-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-3 rounded-xl bg-gradient-to-r from-pink-600 to-purple-600 text-white font-extrabold text-xs shadow-lg flex items-center justify-center gap-2"
              >
                <Instagram className="w-4 h-4" />
                <span>Open Instagram Directly</span>
              </a>

              <button
                type="button"
                onClick={() => setShowInstaQrModal(false)}
                className="w-full py-2 rounded-xl bg-[#240e06] text-amber-300 text-xs font-semibold"
              >
                Close
              </button>
            </div>
